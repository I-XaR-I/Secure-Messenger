import os
import mysql.connector
data = [
    ,
]
inv_s_box = (
    ,
)
round_constant = [
    [0x01, 0, 0, 0],
    [0x02, 0, 0, 0],
    [0x04, 0, 0, 0],
    [0x08, 0, 0, 0],
    [0x10, 0, 0, 0],
    [0x20, 0, 0, 0],
    [0x40, 0, 0, 0],
    [0x80, 0, 0, 0],
    [0x1B, 0, 0, 0],
    [0x36, 0, 0, 0],
]
def rotate_key(key):
    return key[3][1:] + [key[3][0]]

def string_to_hex(key_str):
    if len(key_str) % 16 != 0:
        key_str = key_str.ljust(((len(key_str) // 16) + 1) * 16)
    return [ord(c) for c in key_str]

def substitute_key_bytes(byte_list):
    return [data[b] for b in byte_list]


def xor_with_round_constant(byte_list, round_const):
    return [byte_list[j] ^ round_const[j] for j in range(4)]


def generate_new_key(previous_key, round_const):
    temp = rotate_key(previous_key)
    temp = substitute_key_bytes(temp)
    temp = xor_with_round_constant(temp, round_const)

    new_key = [[], [], [], []]
    for j in range(4):
        new_key[0].append(temp[j] ^ previous_key[0][j])
    for j in range(1, 4):
        for k in range(4):
            new_key[j].append(new_key[j - 1][k] ^ previous_key[j][k])
    return new_key


def generate_roundkeys(initial_key, n=11):
    initial_key = string_to_hex(initial_key)
    initial_key = divide_key(initial_key)
    result = [initial_key]

    for i in range(1, n):
        previous_key = result[-1]
        new_key = generate_new_key(previous_key, round_constant[i - 1])
        result.append(new_key)

    return result

def divide_key(input_key, n=4):
    return [input_key[i : i + n] for i in range(0, len(input_key), n)]



def galois_add(a, b):
    return a ^ b


def galois_mult(a, b):
    result = 0
    irreducible_poly = 0x11B

    while b > 0:
        if b & 1:
            result ^= a

        a <<= 1

        if a & 0x100:
            a ^= irreducible_poly

        b >>= 1

    return result


def inv_substitute_cipher(cipher):
    temp = cipher
    for i in temp:
        for j in range(4):
            for k in range(4):
                i[j][k] = inv_s_box[i[j][k]]
    return temp


def xor_lists(list1, list2):
    return [a ^ b for a, b in zip(list1, list2)]


def addroundkey(cipher, round_key):
    temp = cipher
    temp1 = round_key
    for i in temp:
        for j in range(4):
            i[j] = xor_lists(i[j], temp1[j])
    return temp

def inv_shiftword(cipher):
    for block in cipher:
        block[1] = block[1][3:] + block[1][:3]
        block[2] = block[2][2:] + block[2][:2]
        block[3] = block[3][1:] + block[3][:1]
    return cipher
def inv_mixcolumns(cipher):
    inv_mul_matrix = [[0x0E, 0x0B, 0x0D, 0x09], 
                      [0x09, 0x0E, 0x0B, 0x0D], 
                      [0x0D, 0x09, 0x0E, 0x0B], 
                      [0x0B, 0x0D, 0x09, 0x0E]]

    for block in cipher:
        for col in range(4):
            temp_col = [block[row][col] for row in range(4)]

            for row in range(4):
                temp = 0
                for k in range(4):
                    temp = galois_add(
                        temp, galois_mult(temp_col[k], inv_mul_matrix[row][k])
                    )
                block[row][col] = temp
    return cipher

def decryption(cipher, round_keys, no_of_rounds=10):
    message = cipher
    message = addroundkey(message, round_keys[-1])
    for i in range(-2,-11,-1):
        message = inv_shiftword(message)
        message = inv_substitute_cipher(message)
        message = addroundkey(message, round_keys[i])
        message = inv_mixcolumns(message)
    message = inv_shiftword(message)
    message = inv_substitute_cipher(message)
    message = addroundkey(message, round_keys[0])
    return message


def messageTotext(cipher): 
    resultText=''
    for i in cipher:
        for j in i:
            for k in j:
                resultText+=chr(k)
    return resultText  

def main():
    mydb = mysql.connector.connect(host="localhost",  user="root",  password="Aakash9899" ,database="securemessenger")
    mycursor = mydb.cursor()
    mycursor.execute("SELECT aes_encrypt,aes_key FROM temp")
    data=mycursor.fetchone()
    cipher_matrix = eval(data[0])
    key = data[1]
    round_keys = generate_roundkeys(key)
    message = decryption(cipher_matrix, round_keys)
    decrypted_text = messageTotext(message)
    mycursor.execute(f'update temp set aes_decrypt="{decrypted_text}" where aes_key="{key}"')
    mydb.commit()
    print(decrypted_text)
if __name__ == '__main__':
    main()